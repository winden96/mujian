...
func TestProjectImageGenerationRejectsUnsupportedReferencesBeforeQueue(t *testing.T) {
	...
	_, err = CreateImageGeneration(user.Id, projects[0].ID, CreateImageGenerationInput{
...
func TestProjectImageGenerationFailureUpdatesTaskAndGeneration(t *testing.T) {
	...
	created, err := CreateImageGeneration(user.Id, projects[0].ID, CreateImageGenerationInput{
		SessionID: workspace.ActiveSessionID, Prompt: "失败测试", Engine: "gpt", ModelID: "gpt-image-2", AspectRatio: "16:9",
	})
	require.NoError(t, err)
	failed := waitForImageGeneration(t, user.Id, projects[0].ID, workspace.ActiveSessionID, created.ID, "failed")
