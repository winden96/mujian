...
func setupMujianImageControllerTest(t *testing.T) (model.User, string, string) {
	...
	projects, err := mujianservice.ListProjects(user.Id)
	require.NoError(t, err)
	workspace, err := mujianservice.GetWorkspace(user.Id, projects[0].ID)
	require.NoError(t, err)
	...
	return user, projects[0].ID, workspace.ActiveSessionID
}
